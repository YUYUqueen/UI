...
import useModelToggle, {
  useModelToggleProps,
  useModelToggleEmits
} from '../../composables/private/use-model-toggle.js';
...
import {
  addEscapeKey,
  removeEscapeKey
} from '../../utils/private/escape-key.js';
...
export default defineComponent({
  ...
  setup(props, { slots, emit, attrs }) {
    ...
    const { showPortal, hidePortal, portalIsActive, renderPortal } = usePortal(
      ...
      innerRef,
      renderPortalContent,
      /* pls do check if on a global dialog */ true
    );
    ...
    const classes = computed(
      () =>
        ...
        ` q-dialog__inner--${
          props.maximized === true ? 'maximized' : 'minimized'
        }` +
    ...
    const transitionShow = computed(
      () =>
        'q-transition--' +
        (props.transitionShow === void 0
          ? transitions[props.position][0]
    ...
    const transitionHide = computed(
      () =>
        'q-transition--' +
        (props.transitionHide === void 0
          ? transitions[props.position][1]
    ...
    function renderPortalContent() {
      return h(
        ...
        [
          ...
          h(Transition, { name: transition.value, appear: true }, () =>
            showing.value === true
              ? h(
                  'div',
                  {
