...
import {
  shouldIgnoreKey,
  isKeyCode
} from '../../utils/private/key-composition.js';
...
export default defineComponent({
  ...
  setup(props, { slots, emit }) {
      ...
      return props.options.slice(from, to).map((opt, i) => {
        ...
        if (disable !== true) {
          ...
          itemProps['aria-selected'] =
            itemProps.active === true ? 'true' : 'false';
        ...
        if (
          options[i] === void 0 ||
          isDeepEqual(optionWithoutEvents, options[i]) !== true
        ) {
    ...
    function toggleOption(opt, keepOpen) {
      ...
      const model = props.modelValue.slice(),
        index = innerOptionsValue.value.findIndex(v =>
          isDeepEqual(v, optValue)
        );
    ...
    function onTargetAutocomplete(e) {
      ...
      if (typeof value === 'string' && value.length > 0) {
        ...
        let fn = opt =>
          getOptionValue.value(opt).toLocaleLowerCase() === needle;
    ...
    function getVirtualScrollEl() {
      return hasDialog === true
        ...
        : menuRef.value !== null &&
          menuRef.value.__qPortalInnerRef.value !== null
          ...
          h(
            ...
            () =>
              h('span', {
                ...
                [scope.html === true
                  ? 'innerHTML'
                  : 'textContent']: getOptionLabel.value(scope.opt)
              })
          )
        );
      }
    ...
    function updateMenu(show) {
      ...
      if (show === true) {
        if (innerValue.value.length > 0) {
          ...
          optionIndex = props.options.findIndex(v =>
